...
**现象**：在 Steam 取消订阅后，即使在本管理里启用了 mod，游戏启动时还是会把它清掉，
游戏里也找不到；管理器随后显示「未启用」。

**原因**：`mods/` 目录对 Steam 创意工坊 mod 来说是**托管区** ——
游戏启动时会按订阅状态同步它。所以「取消订阅 + 本管理器启用」这个组合本来**留不住**。

证据（本机实测）：

| 检查点 | 结果 |
|---|---|
| Steam 的安装清单 `steamapps/workshop/appworkshop_250900.acf` | `WorkshopItemsInstalled` 里只剩还订阅着的 2 个 ID |
| 该清单的字段 | 只有 `size` / `timeupdated` / `manifest`，**不含任何路径** → 判断依据就是 mod 的**工坊 ID** |
| `mods/` 目录 | 8 个已取消订阅的 mod 全被清掉，只剩还订阅着的那个 |

**对策：启用时自动「去工坊化」**。把 mod 的 `metadata.xml` 里的工坊 ID 摘掉（写成 `0`），
游戏就把它当成**本地 mod**，不再按订阅状态清理它。原 ID 会记在 mod 目录的
`.imm_workshop_id` 文件里，所以管理器**照旧**能查更新、显示工坊信息、做冲突检测。

- 设置里有开关（默认开启），也能一键「立即处理全部」/「还原 ID」
- 想恢复成官方订阅态，点「还原 ID」即可（`metadata.xml` 会补回原 ID）

> **还在订阅中的 mod** 建议交给 Steam 管（能自动更新）。既订阅又用本工具管理的话，
> 可能出现两份（Steam 展开的实体 + 仓库里的链接），二选一就好。
